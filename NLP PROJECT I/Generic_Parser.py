...
def get_data(doc):
    ...
    for token in tokens:
        if not token.isdigit() and len(token) > 1 and token in lm_dictionary:
            ...
            if lm_dictionary[token].positive: _odata[7] += 1
            if lm_dictionary[token].negative: _odata[8] += 1
            if lm_dictionary[token].uncertainty: _odata[9] += 1
            if lm_dictionary[token].litigious: _odata[10] += 1
            if lm_dictionary[token].weak_modal: _odata[11] += 1
            if lm_dictionary[token].moderate_modal: _odata[12] += 1
            if lm_dictionary[token].strong_modal: _odata[13] += 1
            if lm_dictionary[token].constraining: _odata[14] += 1
