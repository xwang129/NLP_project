...
def load_masterdictionary(file_path, print_flag=False, f_log=None, get_other=False):
    ...
    :param file_path: path for file LoughranMcDonald_MasterDictionary_2018.csv
    :param print_flag:
    :param f_log:
    :param get_other: if this function return _master_dictionary along with other variables?
    :return:
    """
    _master_dictionary = {} # load the whole dictionary into this variable. key is word cols[0], value is
    # MasterDictionary object defined below
    ...
    with open(file_path) as f:
        _total_documents = 0  #
        _md_header = f.readline()  # consume the first/header line
        for line in f:
            cols = line.split(',')
            _master_dictionary[cols[0]] = MasterDictionary(cols, _stopwords)
            # “Document Count” indicates
            # the number of 10-X filings containing at least one occurrence of the word.
...
def create_sentimentdictionaries(_master_dictionary, _sentiment_categories):
    """
    create sentiment dictionary from loaded _master_dictionary
    :param _master_dictionary:
    :param _sentiment_categories: list of sentiment categories
    :return: _sentiment_dictionary, key is a category, value is also a dictionary, key is word that belongs to this
    # sentiment, value is 0
    """
...
class MasterDictionary:
    # this class holds one row of LoughranMcDonald_MasterDictionary_2018.csv file
    # each row is a word
    def __init__(self, cols, _stopwords):
        """

        :param cols: a row of CSV file above, each column corresponds to a variable
        :param _stopwords: a list of stop words
        """
...
if __name__ == '__main__':
    ...
    md = r'G:\NLP-P1-Textdata\dictionary\LoughranMcDonald_MasterDictionary_2018.csv'
    master_dictionary, md_header, sentiment_categories, stopwords, total_documents \
